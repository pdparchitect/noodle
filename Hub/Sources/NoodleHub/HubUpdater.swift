...
@MainActor final class HubUpdater: NSObject, ObservableObject {
    ...
    func setAutomaticChecks(_ enabled: Bool) { if started { controller.updater.automaticallyChecksForUpdates = enabled } }
    func setAutomaticDownloads(_ enabled: Bool) { if started { controller.updater.automaticallyDownloadsUpdates = enabled } }
}

extension HubUpdater: SPUUpdaterDelegate {
...
struct HubUpdatesSettingsView: View {
        ...
        Form {
            ...
            Section {
                ...
                Toggle("Automatically download and install updates",
                       isOn: Binding(get: { updater.automaticallyDownloads }, set: updater.setAutomaticDownloads))
