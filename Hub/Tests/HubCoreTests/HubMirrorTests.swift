...
@MainActor final class HubMirrorTests: XCTestCase {
    ...
    private let calendar = ToolProviderListing(
        manifest: ToolProviderManifest(id: "calendar", title: "Calendar", summary: "Read the calendars assigned to this bot."),
        tools: Data(#"{"tools":[{"name":"events","description":"List events.","inputSchema":{"type":"object"}}]}"#.utf8))

    private func callTool(in workspace: URL) async throws -> Data {
        try await Task.detached {
            try ToolBridgeClient.request(.call, provider: "calendar", tool: "events", arguments: Data("{}".utf8),
                                         workspace: workspace, currentDirectory: workspace)
        }.value
    }

    func testABotOnTheHubUsesToolsFromTheMacThatLendsThem() async throws {
        let f = try await fixture()
        let mirror = f.mirror()
        let agent = try await mirror.createBot(LinkBotDraft(name: "Alfred", provider: "claude-code"))
        let calendar = self.calendar
        mirror.toolCatalogue = { _ in [calendar] }
        mirror.toolRunner = { localID, request in
            XCTAssertEqual(localID, agent.id)
            XCTAssertEqual(request.provider, "calendar")
            return Data(#"{"content":[{"type":"text","text":"3 events"}]}"#.utf8)
        }
        try f.hub.bots.startTools()
        addTeardownBlock { await MainActor.run { f.hub.bots.stop() } }
        let running = Task { await mirror.run() }
        addTeardownBlock { running.cancel() }
        let workspace = f.hub.repository.directory(for: try XCTUnwrap(f.hub.repository.loadAgents().first))
        let skill = workspace.appendingPathComponent(".agents/skills/calendar/SKILL.md")
        for _ in 0..<50 where !FileManager.default.fileExists(atPath: skill.path) { try await Task.sleep(for: .milliseconds(100)) }
        XCTAssertTrue(try String(contentsOf: skill, encoding: .utf8).contains("events"))
        XCTAssertTrue(try String(contentsOf: workspace.appendingPathComponent("AGENTS.md"), encoding: .utf8).contains("calendar"))

        let result = try await callTool(in: workspace)
        XCTAssertEqual(String(decoding: result, as: UTF8.self), #"{"content":[{"type":"text","text":"3 events"}]}"#)
    }

    func testToolsFailPlainlyWhileTheMacIsAway() async throws {
        let f = try await fixture()
        let mirror = f.mirror()
        _ = try await mirror.createBot(LinkBotDraft(name: "Alfred", provider: "claude-code"))
        let calendar = self.calendar
        mirror.toolCatalogue = { _ in [calendar] }
        await mirror.sync()
        try f.hub.bots.startTools()
        ...
        let workspace = f.hub.repository.directory(for: try XCTUnwrap(f.hub.repository.loadAgents().first))
        do {
            _ = try await callTool(in: workspace)
            XCTFail("A tool ran with its Mac away")
        } catch {
            XCTAssertEqual(error.localizedDescription, "Mac is not connected to this Hub, so its tools are unavailable.")
        }
    }
}
