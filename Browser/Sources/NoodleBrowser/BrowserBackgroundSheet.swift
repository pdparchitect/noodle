...
struct BrowserBackgroundSheet: View {
    let original: ConversationBackground
    let originalFile: PreparedBackgroundFile?
    let imageURL: URL?
    ...
    init(background: ConversationBackground, imageURL: URL?, file: PreparedBackgroundFile? = nil,
        ...
        original = background; originalFile = file; self.imageURL = imageURL; self.onApply = onApply
        _selected = State(initialValue: background)
        _preparedFile = State(initialValue: file)
    }
            ...
            HStack {
                ...
                Button("Apply") {
                    ...
                    Task {
                        defer { busy = false }
                        do {
                            try onApply(selected, preparedFile)
                            dismiss()
                ...
                .disabled(busy || (selected == original && preparedFile == originalFile))
            }.buttonStyle(.plain)
            ConversationBackgroundView(background: selected,
                imageURL: preparedFile?.url ?? (selected.imageFilename == nil ? nil : imageURL))
