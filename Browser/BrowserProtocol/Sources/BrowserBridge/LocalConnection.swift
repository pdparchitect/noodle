...
public enum BrowserConnection {
    public static var providerID: String { BrowserBuildIdentity.current.providerID }
    public static var clientIDs: [String] { BrowserBuildIdentity.current.clientIDs }
    public static let maxFrame = 32 * 1_048_576

    public static func socketURL(bundle: Bundle = .main) throws -> URL {
        guard let identity = BrowserBuildIdentity.identify(bundle.bundleIdentifier),
              let group = bundle.object(forInfoDictionaryKey: "NoodleBrowserGroup") as? String else {
            throw BrowserError("Browser integration is not configured in this signed build.")
        }
        try identity.validateGroup(group, team: signingTeam(bundle: bundle))
        guard identity == BrowserBuildIdentity.processIdentity else { throw BrowserError("Browser environment mismatch.") }
        guard let root = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: group) else {
            throw BrowserError("The Browser connection container is unavailable.")
        }
        return root.appendingPathComponent("b.sock")
    }
    public static func signingTeam(bundle: Bundle = .main) throws -> String {
        guard let team = bundle.object(forInfoDictionaryKey: "NoodleSigningTeam") as? String,
            ...
            throw BrowserError("Browser integration requires a team-signed build.")
        }
        return team
    }
    ...
    static func authenticate(_ fd: Int32, team: String, identifiers: [String]) throws -> String {
        ...
        guard getsockopt(fd, SOL_LOCAL, LOCAL_PEERTOKEN, &token, &size) == 0 else {
            throw BrowserError("Cannot authenticate the browser connection.")
        }
        ...
        let lookup = SecCodeCopyGuestWithAttributes(
            ...
            [
                ...
                kSecGuestAttributeDynamicCode: true,
            ] as CFDictionary, [], &code)
        guard lookup == errSecSuccess,
            let code
        else { throw BrowserError("Untrusted browser connection (\(lookup)).") }
        ...
        throw BrowserError("The peer is not an authorized Noodle application (\(lastStatus)).")
    }
        ...
        try data.withUnsafeMutableBytes { buffer in
            ...
            while offset < count {
                let received = Darwin.read(
                    fd, buffer.baseAddress!.advanced(by: offset), count - offset)
                if received < 0 && errno == EINTR { continue }
                guard received > 0 else {
                    throw BrowserError(
                        "Browser disconnected or timed out. Check its state before retrying.")
                }
                offset += received
            }
        }
    ...
    static func receive(_ fd: Int32) throws -> Data {
        ...
        guard count > 0, count <= maxFrame else {
            throw BrowserError("Invalid browser message size.")
        }
        return try read(fd, count: count)
    }
    static func send(_ data: Data, _ fd: Int32) throws {
        guard !data.isEmpty, data.count <= maxFrame else {
            throw BrowserError("Browser response is too large.")
        }
        ...
        try packet.withUnsafeBytes { buffer in
            ...
            while offset < buffer.count {
                ...
                guard written > 0 else { throw BrowserError("Browser connection write failed.") }
                offset += written
            }
        }
    }
    public static func call(
        ...
        return try await Task.detached(priority: .userInitiated) {
            ...
            do { try send(JSONEncoder().encode(request), fd) } catch {
                if let reply = try? JSONDecoder().decode(BrowserResponse.self, from: receive(fd)),
    ...
    public init(
        ...
        if lstat(url.path, &info) == 0 {
            guard info.st_mode & S_IFMT == S_IFSOCK, info.st_uid == getuid() else {
                throw BrowserError("Unsafe browser connection path.")
            }
            ...
            guard unlink(url.path) == 0 else {
                throw BrowserError("Cannot replace stale browser connection.")
            }
        }
        let fd = socket(AF_UNIX, SOCK_STREAM, 0)
        ...
        guard BrowserConnection.withAddress(&address, { Darwin.bind(fd, $0, $1) }) == 0,
            ...
            Darwin.close(fd)
            throw BrowserError("Cannot register the browser provider (\(errno)).")
        }
        ...
        source.setEventHandler {
            ...
            BrowserConnection.configure(peer)
            DispatchQueue.global(qos: .userInitiated).async {
                do {
                    ...
                    try? BrowserConnection.send(
                        JSONEncoder().encode(BrowserResponse(error: error.localizedDescription)),
                        peer)
                    Darwin.close(peer)
                    permits.signal()
                }
