...
public enum AppletConnection {
    ...
    public static func signingTeam(bundle: Bundle = .main) throws -> String {
        guard let team = bundle.object(forInfoDictionaryKey: "NoodleSigningTeam") as? String,
            ...
            throw AppletError("Applet integration requires a team-signed build.")
        }
        return team
    }
    static func address(_ url: URL) throws -> sockaddr_un {
        ...
        guard path.count <= MemoryLayout.size(ofValue: address.sun_path) else {
            throw AppletError("Applet connection path is too long.")
        }
    ...
    static func authenticate(_ fd: Int32, team: String, identifiers: [String]) throws -> String {
        ...
        guard getsockopt(fd, SOL_LOCAL, LOCAL_PEERTOKEN, &token, &size) == 0 else {
            throw AppletError("Cannot authenticate the applet connection.")
        }
        ...
        guard lookup == errSecSuccess,
            let code
        else { throw AppletError("Untrusted applet connection (\(lookup)).") }
        ...
        throw AppletError("The peer is not an authorized Noodle application (\(lastStatus)).")
    }
        ...
        try data.withUnsafeMutableBytes { buffer in
            ...
            while offset < count {
                ...
                guard received > 0 else {
                    throw AppletError(
                        "Applet disconnected or timed out. Check its state before retrying.")
                }
                offset += received
            }
        }
    ...
    static func send(_ data: Data, _ fd: Int32) throws {
        ...
        try packet.withUnsafeBytes { buffer in
            ...
            while offset < buffer.count {
                ...
                guard written > 0 else { throw AppletError("Applet connection write failed.") }
                offset += written
            }
        }
    }
    public static func call(
        ...
        return try await Task.detached(priority: .userInitiated) {
            ...
            do { try send(JSONEncoder().encode(request), fd) } catch {
                if let reply = try? JSONDecoder().decode(AppletResponse.self, from: receive(fd)),
                    let message = reply.error
                {
                    throw AppletError(message)
                }
                throw error
            }
    ...
    public init(
        ...
        if lstat(url.path, &info) == 0 {
            guard info.st_mode & S_IFMT == S_IFSOCK, info.st_uid == getuid() else {
                throw AppletError("Unsafe applet connection path.")
            }
            let probe = socket(AF_UNIX, SOCK_STREAM, 0)
            guard probe >= 0 else { throw AppletError("Cannot inspect applet connection.") }
            let active =
                AppletConnection.withAddress(&address, { Darwin.connect(probe, $0, $1) }) == 0
            Darwin.close(probe)
            guard !active else { throw AppletError("A applet provider is already running.") }
            guard unlink(url.path) == 0 else {
                throw AppletError("Cannot replace stale applet connection.")
            }
        }
        let fd = socket(AF_UNIX, SOCK_STREAM, 0)
        guard fd >= 0 else { throw AppletError("Cannot create applet connection.") }
        AppletConnection.configure(fd)
        guard AppletConnection.withAddress(&address, { Darwin.bind(fd, $0, $1) }) == 0,
            chmod(url.path, 0o600) == 0, Darwin.listen(fd, 16) == 0
        else {
            Darwin.close(fd)
            throw AppletError("Cannot register the applet provider (\(errno)).")
        }
        ...
        source.setEventHandler {
            ...
            AppletConnection.configure(peer)
