...
private final class CodexAgentProcess {
    ...
    Messenger also supports `--react --conversation <uuid> --message <message-uuid> --emoji '👀'` and `--unreact` to remove your own emoji. Use reactions when useful for acknowledgement, progress, completion, or feedback, keeping progress indicators accurate. `--list-messages --conversation <uuid>` reads history including your own messages and current reactions without consuming the inbox. `--list-participants --conversation <uuid>` returns the current conversation roster, each bot's public description, and its most recent activity in that conversation; it never exposes private backstories. Inbox deliveries with `reactionChange` are feedback on the referenced message, not a new instruction to repeat it. The change's `sender` names the reactor; `emoji` and `removed` describe the update. Do not reply to every reaction or create acknowledgement loops.
