...
public enum MessengerDocumentation {
    ...
    public static var referenceMarkdown: String {
        ...
        Apple includes up to 1,600 characters each from the backstory and `preferences.md` on every wake. Newer explicit user requests take precedence over standing preferences.

        Every Apple turn exposes exactly `bash`, `read`, and `write`, including text chat, images, and background events. Bash runs in the bot workspace under its existing access policy. There is no request classifier or dedicated conversation-history or Messenger model tool. Read `AGENTS.md` for workspace guidance and assigned tools, then use the corresponding `.agents/skills/*/SKILL.md` and shared CLI for Messenger, MCP integrations, Computer, and Applet operations. Tool assignments and broker permissions still apply.
