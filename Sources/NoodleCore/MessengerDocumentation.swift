...
public enum MessengerCommandKind: String, CaseIterable, Sendable {
    ...
    public var usage: String {
        ...
        case .send: return "--send --conversation <uuid> [--body <text> | --body-percent-encoded <utf8> | --body-base64 <utf8-base64>] [--attach <file-path> ...]"
        }
    }
...
public enum MessengerDocumentation {
    ...
    Use the bundled Messenger CLI from the bot workspace. In Codex, immediately read an inbox notification through the programmatic bridge: `const r = await tools.exec_command({cmd: "./.agents/skills/messenger/messenger --get-latest --inline-images", max_output_tokens: 250000}); if (r.exit_code !== 0) throw new Error(r.output); const payload = JSON.parse(r.output); text(payload.deliveries); for (const visual of payload.images) image(visual.dataURL, "original");`. In Claude Code, run `./.agents/skills/messenger/messenger --get-latest` with Bash and inspect image attachments using Claude's Read tool on their absolutePath. Every delivery names me and supplies named `participants` (the named participant roster), conversation context and sender identity. Every attachment includes its exact absolutePath for file work. Run get-latest only once per notification because it consumes the inbox.
