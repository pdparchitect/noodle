...
public struct ConnectionToolProvider: ToolProvider {
    ...
    static func guidance(id: String, userInstructions: String) -> String {
        ...
        For loops, filtering and chained calls, use `\(command) --run FILE`, `--run -` (JavaScript on standard input) or `--eval CODE`. Scripts use macOS JavaScriptCore with synchronous mcp.tools(), mcp.inspect(name), mcp.call(name, input = {}), mcp.resources() and mcp.readResource(uri). Results are JavaScript objects with the same fields as the JSON above. call and readResource accept a final {raw: true} option. print(value) writes one JSON line to standard output; console.log/info/warn/error/debug/dir write diagnostics to standard error, console.trace() includes a stack, and console.assert(condition, ...values) logs failed assertions without throwing. Nothing prints implicitly. Errors throw; tool errors keep the full result as error.result. Uncaught errors print source locations and exit nonzero. Each run starts fresh: no imports, Node or browser APIs, shell access or async workflows. Script files must be UTF-8 regular workspace files without links or '..', at most 1 MiB. Limits: 100 operations, 8 MiB combined output, 300 seconds including calls; --timeout SECONDS accepts 1–3600. A timeout stops the script, but remote actions may already have completed.
