...
- Remove the separate `browser` command and its hand-written skill. Bots use `messenger tool browser` with the same tool and option names, including `present`; Noodle removes the old skill and its request folder from each bot's workspace.
