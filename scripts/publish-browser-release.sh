...
version="${1:?Pass the Browser version}"
notes="${2:?Pass the release notes file}"
[[ "$version" =~ '^[0-9]+\.[0-9]+\.[0-9]+$' ]] || exit 1
[[ "$version" == "$(tr -d '[:space:]' < "$project_root/Browser/VERSION")" ]] || exit 1
tag="browser-v$version"
repo="pdparchitect/noodle"
channel=browser-latest
assets="$project_root/dist/browser-$version"
archive="Noodle-Browser-arm64.zip"
...
if [[ -n "$channel_title" ]]; then
    previous="${channel_title#Noodle Browser }"
    [[ "$previous" =~ '^[0-9]+\.[0-9]+\.[0-9]+$' ]] || { print -u2 'Unrecognized Browser channel version'; exit 1; }
    autoload -Uz is-at-least
    is-at-least "$previous" "$version" || { print -u2 'Refusing to roll back the Browser update channel'; exit 1; }
fi
gh release create "$tag" "$assets/$archive" "$assets/$archive.sha256" "$assets/$disk_image" "$assets/$disk_image.sha256" "$assets/appcast.xml" \
    --repo "$repo" --verify-tag --draft --latest=false --title "Noodle Browser $version" --notes-file "$notes"
...
channel_notes="Noodle Browser $version
...
Requires macOS 26 or later. Open the DMG and drag Noodle Browser.app to Applications. Noodle and Browser must be signed by the same publisher.

[Release notes and checksum](https://github.com/$repo/releases/tag/$tag)

Updating or quitting Browser closes live pages. Website sign-ins and tab URLs persist; save unsent forms before updating.
"
if [[ -z "$channel_title" ]]; then
    gh release create "$channel" "$assets/$archive" "$assets/$archive.sha256" "$assets/$disk_image" "$assets/$disk_image.sha256" "$assets/appcast.xml" \
        ...
        --draft --latest=false --title "Noodle Browser $version" --notes "$channel_notes"
    ...
    gh release edit "$channel" --repo "$repo" --latest=false --title "Noodle Browser $version" --notes "$channel_notes"
    ...
    previous_archive="Noodle-Browser-$previous-arm64.zip"
