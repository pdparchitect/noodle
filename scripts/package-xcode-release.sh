#!/bin/zsh
# Builds, signs, notarizes and packages a public release of an app built from APP/Project.swift.
# Everything else follows from the app's name: APP/VERSION and APP/CHANGELOG.md, the app-v tag,
# com.pdparchitect.noodle.app, Noodle-APP-arm64.zip and scripts/verify-app-release.sh.
set -euo pipefail
project_root="${0:A:h:h}"
app_folder="${1:?Usage: scripts/package-xcode-release.sh APP TAG}"
[[ -f "$project_root/$app_folder/Project.swift" ]] || { print -u2 "$app_folder has no Project.swift."; exit 1; }
product="${(L)app_folder}"
setting="${(U)app_folder}"
version="$(tr -d '[:space:]' < "$project_root/$app_folder/VERSION")"
...
grep -Eq "^## \\[$version\\] - [0-9]{4}-[0-9]{2}-[0-9]{2}$" "$project_root/$app_folder/CHANGELOG.md" || {
...
(cd "$project_root/$app_folder" && "$tuist" generate --no-open >&2)
derived="$project_root/.build/$product-release"
app_name="Noodle $app_folder"
xcodebuild -workspace "$project_root/$app_folder/Noodle$app_folder.xcworkspace" -scheme "Noodle$app_folder" \
    -configuration Release -derivedDataPath "$derived" -destination 'generic/platform=macOS' \
    -archivePath "$staging/Noodle$app_folder.xcarchive" -skipPackagePluginValidation -skipMacroValidation \
...
ditto "$staging/Noodle$app_folder.xcarchive/Products/Applications/$app_name.app" "$app"
[[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$app/Contents/Info.plist")" == "com.pdparchitect.noodle.$product" ]]
...
archive="Noodle-$app_folder-arm64.zip"
