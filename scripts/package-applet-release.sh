...
version="$(tr -d '[:space:]' < "$project_root/Applet/VERSION")"
...
grep -Eq "^## \\[$version\\] - [0-9]{4}-[0-9]{2}-[0-9]{2}$" "$project_root/Applet/CHANGELOG.md" || {
    print -u2 'Applet release notes are not approved and dated.'; exit 1
}
...
staging="$(mktemp -d "$project_root/.release/applet.XXXXXX")"
...
zsh "$project_root/scripts/verify-applet-release.sh" "$app"
