...
# NOODLE_DATA_CONTAINER. Development is the Dev app (Debug), production the released one (Release).
...
case "${(P)container_setting:-${NOODLE_DATA_CONTAINER:-development}}" in
    ...
    *) print -u2 "$container_setting must be development or production."; exit 1 ;;
esac
