...
smoke_id="$(uuidgen)"
artifacts="$project_root/.build/browser-webmcp-verification/$smoke_id"
mkdir -p "$artifacts"
fixture_pid=''; browser_pid=''; port=''
cleanup() {
...
fixture_args=(--smoke-test --smoke-id "$smoke_id" --smoke-port "$port" --webmcp-only)
...
if [[ -n "${NOODLE_BROWSER_TEST_NOODLE_APP:-}" ]]; then
    actual_identity="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$NOODLE_BROWSER_TEST_NOODLE_APP/Contents/Info.plist")"
    [[ "$actual_identity" == "$broker_identity" ]] || { print -u2 'Use matching Dev or normal test bundles.'; exit 1; }
    ...
    "$NOODLE_BROWSER_TEST_NOODLE_APP/Contents/MacOS/Noodle" --browser-integration-test --webmcp-only --browser-fixture "$browser_id" --browser-fixture-port "$port" > "$artifacts/broker.log" 2>&1 || { cat "$artifacts/broker.log"; exit 1; }
    cat "$artifacts/broker.log"
    kill "$browser_pid"; wait "$browser_pid" 2>/dev/null || true; browser_pid=''
fi
"$executable" --smoke-test --cleanup --smoke-id "$smoke_id" --smoke-port "$port" > "$artifacts/cleanup.log" 2>&1
[[ ! -e "$profile_root" ]] || { print -u2 'Test profile cleanup did not complete.'; exit 1; }
port=''
print "WebMCP verification artifacts: $artifacts"
