...
version="$(tr -d '[:space:]' < "$project_root/Browser/VERSION")"
...
grep -Eq "^## \\[$version\\] - [0-9]{4}-[0-9]{2}-[0-9]{2}$" "$project_root/Browser/CHANGELOG.md" || {
    print -u2 'Browser release notes are not approved and dated.'; exit 1
}
...
staging="$(mktemp -d "$project_root/.release/browser.XXXXXX")"
trap 'rm -rf "$staging"' EXIT
output="$project_root/dist/browser-$version"
[[ ! -e "$output" ]] || { print -u2 "Release output already exists: $output"; exit 1; }
[[ "$(uname -m)" == arm64 ]] || { print -u2 'Public Browser archives require Apple silicon.'; exit 1; }
export NOODLE_BROWSER_CONFIGURATION=release NOODLE_BROWSER_DATA_CONTAINER=production
export NOODLE_REQUIRE_DEVELOPER_ID=1 NOODLE_CODESIGN_TIMESTAMP=1
app="$(zsh "$project_root/scripts/build-browser.sh")"
[[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$app/Contents/Info.plist")" == com.pdparchitect.noodle.browser ]]
codesign --verify --deep --strict "$app"
codesign -dv --verbose=4 "$app" 2>&1 | grep -q '^Authority=Developer ID Application:'
zsh "$project_root/scripts/verify-browser.sh" "$app"
...
sparkle_tools="$project_root/.build/browser/artifacts/sparkle/Sparkle/bin"
