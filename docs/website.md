...
package installation, rendering library, or build step. Only that directory is
...
The cue fades away as scrolling starts and its shimmer pauses while hidden.
It is omitted from reduced-motion and static views. Once the screen is illuminated,
...
the download button sits directly below the headline. The laptop settles below
the copy, and scrolling backward hides the text again. Reduced
...
The still alternatives retain their original screenshot. Scroll progress drives
a bounded animation-frame loop; projected geometry is fitted to the viewport as the lid
...
preferences show the open laptop without the long scroll sequence. Browsers
without CSS 3D or JavaScript get a static screenshot.

### Archived model experiment

The CSS version is the chosen direction. Git checkpoint `c085bc7` preserves the
earlier 3D experiment: `comparison.html`, `model.html`, `model.js`, the model
archive, its preparation script, and the vendored Three.js library and license.
These files were removed from the active website after the checkpoint. The
keyboard measurements and vector-label preparation script remain independent
of that runtime. The user-supplied ZIP and USDZ remain external reference files.
