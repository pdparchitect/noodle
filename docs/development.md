...
The script checks menu title formatting, description truncation, circular image transparency, generated colours/symbols and invalid-image fallback before opening the fixture. Failures exit with a diagnostic instead of crashing the UI. The fixture uses the real shared avatar renderer. Its description toggle uses the fixture's isolated preferences; check the toggle defaults on and an explicitly saved off choice survives reopening. In Noodle, the same toggle lives in General settings. Menu selection must insert only the name, even with descriptions enabled.
