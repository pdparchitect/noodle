...
Bots use a signed `NoodleAgentHost.xpc` by default so they can run autonomously outside Noodle's App Sandbox. Settings → Security can explicitly restrict a Codex bot to its private workspace instead; Claude Code currently requires autonomous mode. The helper has its own process boundary, runs as the current user without App Sandbox or extra entitlements, and accepts only the main app's exact signing identity/team. The app likewise verifies the helper. Autonomous mode uses a vendor-signed Codex or Claude Code executable, a validated UUID workspace, and a provider-specific fixed argument set—never a client-selected shell or arbitrary command.
