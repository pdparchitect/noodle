...
As with the other external native adapters, Muse needs explicit per-bot autonomous access. Restricted bots do not launch it. The host selects fixed `serve --disable-sandbox --trust-workspace` arguments only for that opt-in path. Muse's startup approval policy stays intact; Noodle accepts only an offered once-only approval for the current session and requirement stage. Unsupported approvals and interactive input requests fail visibly rather than leaving a spinner indefinitely. App and helper entitlements are unchanged. Sessions persist, reopen through `session/resume`, and receive inbox, heartbeat and recovery events. Acknowledging `turn/start` does not complete a turn: only its matching `turn/completed` event clears unfinished work. Messenger remains the only author of chat messages.
